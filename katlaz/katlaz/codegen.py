...
try:
    from .parser import Print, Return, Var, FuncDef
except Exception:
    Print = Return = Var = FuncDef = None
...
class CodeGen:
    ...
    def expr(self, node):
        ...
        if isinstance(node, MapLiteral):
            ...
            return tmp
        if Var is not None and isinstance(node, Var):
            return node.name
...
# Compatibility generators added by fixer
def _gen_FuncDef(self, node):
    ret_type = self.map_type(node.return_type) if node.return_type else "void"
    args = ", ".join([f"{self.map_type(a['type'] if isinstance(a, dict) else a.var_type)} {a['name'] if isinstance(a, dict) else a.name}" for a in node.args])
    self.emit(f"{ret_type} {node.name}({args}) {{")
    self.indent += 1
    for stmt in node.body:
        self.generate(stmt)
...
CodeGen.gen_FuncDef = _gen_FuncDef
CodeGen.gen_Print = _gen_Print
CodeGen.gen_Return = _gen_Return
CodeGen.gen_If = _gen_If
CodeGen.gen_While = _gen_While
